const s={gmail:{host:"smtp.gmail.com",port:587,secure:!1},outlook:{host:"smtp-mail.outlook.com",port:587,secure:!1},yahoo:{host:"smtp.mail.yahoo.com",port:587,secure:!1},custom:{host:"",port:587,secure:!1}},d=(e,o,t)=>`
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Application Confirmation - Unlock Uzbekistan</title>
    <style>
        body {
...
<body>
    <div class="container">
        ...
        <div class="footer">
            <p style="margin: 0;">© 2024 Unlock Uzbekistan. All rights reserved.</p>
...
This is an automated confirmation email.
  `,m=()=>{const e="custom",o="no-reply@unlockadmissions.uz",t="yozmalarinzb",i=s[e]||s.gmail;return{host:"de.ahost.cloud",port:parseInt("465"),secure:i.secure,username:o,password:t,fromName:"UNLOCK Admissions",fromEmail:"no-reply@unlockadmissions.uz"}},u=async e=>{try{const o=m();if(!o)return{success:!1,error:"SMTP configuration not found. Please set VITE_SMTP_USERNAME and VITE_SMTP_PASSWORD."};const t=d(e.applicantName,e.applicationId,e.submissionDate),i=l(e.applicantName,e.applicationId,e.submissionDate),n={smtp:o,to:e.to,subject:"Application Confirmation - Welcome to Unlock Uzbekistan!",html:t,text:i},a=await fetch("/api/send-email",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!a.ok){const c=await a.json().catch(()=>({message:"Unknown error"}));throw new Error(`SMTP API error: ${c.message||a.statusText}`)}const r=await a.json();return console.log("SMTP confirmation email sent successfully:",r),{success:!0,messageId:r.messageId}}catch(o){return console.error("Failed to send SMTP confirmation email:",o),{success:!1,error:o instanceof Error?o.message:"Unknown SMTP error"}}},p=async e=>{try{const o="Application Confirmation - Welcome to Unlock Uzbekistan!",t=l(e.applicantName,e.applicationId,e.submissionDate),i=`mailto:${e.to}?subject=${encodeURIComponent(o)}&body=${encodeURIComponent(t)}`;return{success:!1,error:"Browser email not configured for production"}}catch(o){return console.error("Failed to send browser email:",o),{success:!1,error:o instanceof Error?o.message:"Unknown browser email error"}}},g=async(e,o,t)=>{const i={to:e,applicantName:o,applicationId:t,submissionDate:new Date().toLocaleDateString("en-US",{year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit"})};let n=await u(i);return n.success||(console.log("SMTP failed, trying browser email fallback..."),n=await p(i)),n};export{g as sendApplicationConfirmation,p as sendBrowserEmail,u as sendSMTPConfirmationEmail};
