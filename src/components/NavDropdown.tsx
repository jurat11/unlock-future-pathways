...
const NavDropdown = ({
  ...
  return <div className="relative" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} ref={dropdownRef}>
      ...
      {/* Dropdown Menu */}
      <div className={cn("absolute top-full left-0 mt-2 w-56 sm:w-64 bg-white border border-gray-200 rounded-2xl shadow-lg transform transition-all duration-400 ease-out overflow-hidden", "z-[100] before:content-[''] before:absolute before:-top-1 before:left-6 before:w-2 before:h-2 before:bg-white before:border-l before:border-t before:border-gray-200 before:rotate-45", isOpen ? "opacity-100 translate-y-0 scale-100 pointer-events-auto" : "opacity-0 translate-y-3 scale-95 pointer-events-none")}>
